...
Ext.onReady(function() {
	...
	var planstore = new Ext.data.SimpleStore({
		fields : ['name', 'code'],
		data : [['A计划', '1'], ['B计划', '2'],['C计划', '3']]
	});
	
	var planCombo= new Ext.form.ComboBox({
		id : 'plan',
		hiddenName : 'plan_name',
		fieldLabel : '间隔计划',
		emptyText : '请选择间隔计划...',
		triggerAction : 'all',
		store : planstore,
	...
	var qForm = new Ext.form.FormPanel({
		...
		buttons : [{
			...
			var cm = new Ext.grid.ColumnModel([rownum, sm,   {
						...
						header : '高峰计划',
						dataIndex : 'inteval_avg_g',
						width : 70	
						},{
						header : '平峰计划',
						dataIndex : 'inteval_avg_p',
						width : 70	
						},{
						header : '标准倍数',
						dataIndex : 'inteval_vag',
						width : 70	
						}
					]);
			...
			var store = new Ext.data.Store({
						// 获取数据的方式
						proxy : new Ext.data.HttpProxy({
									url :'busruntime.do?reqCode=busPeak_View'
								}),
						// 数据读取器
						reader : new Ext.data.JsonReader({},[ {
											name : 'route'
