		...
		buttons : [{
			...
			var store = new Ext.data.Store({
						...
						proxy : new Ext.data.HttpProxy({
									url :'busbanci.do?reqCode=queryCompanyFlatPeakInterval'
								}),
						// 数据读取器
			...
			// 不产横向生滚动条, 各列自动扩展自动压缩, 适用于列数比较少的情况
				...
				title : '<span class="commoncss">路队信息<span>', // 窗口标题
				layout : 'fit', // 设置窗口布局模式
				width : 900, // 窗口宽度
				height : 300, // 窗口高度
			...
			grid.on('rowdblclick', function(pGrid, rowIndex, event) {
						...
						var record = pGrid.getStore().getAt(rowIndex);
						// 获取单元格数据集
						var comp_id = record.get("comp_id");
						//alert(data);
						var params = qForm.getForm().getValues();
						params.companyName=comp_id;
						showCatalogItem(params);
					//	Ext.MessageBox.alert('提示', "双击行的索引为:" + rowIndex);
					});
					
					// 监听行双击事件
			grid1.on('rowdblclick', function(pGrid, rowIndex, event) {
						// 获取行数据集
						var record = pGrid.getStore().getAt(rowIndex);
						// 获取单元格数据集
			...
			function showCatalogItem(record){
			...
			function showCatalogItem2(record){
